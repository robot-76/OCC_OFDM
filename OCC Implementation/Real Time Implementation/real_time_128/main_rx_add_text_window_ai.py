...
from frame_decode_ai import process_image # Import the frame processing function
...
# Initialize previous messages for comparison
previous_rx_messages = None 
...
while camera.IsGrabbing():
    ...
    if grab_result.GrabSucceeded():
        ...
        # Check if rx_messages is the same as the previous one, if so, skip processing
        if previous_rx_messages is not None and rx_messages == previous_rx_messages:
            grab_result.Release()
            continue  # Skip this frame and move to the next one

        previous_rx_messages = rx_messages  # Update previous messages for next comparison
        ...
        cv2.imshow("Extracted Messages (with AI)", text_window)
