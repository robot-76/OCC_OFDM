...
def process_image(image_data): # directly data array is transfered fron another .py file
    ...
    # columns_range_data = image_data[:, 500:1200]
    ...
    for i, batch in enumerate(batches_column_avg):
        ...
        if output_rx is not None:
                if len(output_rx) == batch_size:
                    ...
                    rx_reshaped = prediction.reshape(-1, 104)
                    clipped_data = []
                    ...
                    clip_rx_data = clipped_data.reshape(-1,104)
                    ...
                    try:
                        ...
                        if decoded_message and decoded_message != previous_decoded_message:  
                            # Only append if it's not the same as the previous message
                            decoded_messages.append(decoded_message)  
                            previous_decoded_message = decoded_message  # Update the previous message tracker
